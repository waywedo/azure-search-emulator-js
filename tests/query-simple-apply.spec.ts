...
describe("query-simple", () => {
    describe("apply", () => {
        const simpleAnalyzer: AnalyzerFn = createTokenizer({
            splitter: /(\s+)/
        });
        ...
        function itShouldScore(
            ...
            const exp = expected.map((e) => ({
                ...
                ngrams: e.ngrams,
                score:
                    (1 - e.range[0] / entry.length) * (e.range[1] - e.range[0])
            }));

            it(`should score ${query} ${searchMode} ${entry}`, () => {
                const strat = createSimpleQueryStrategy({
                    search: query,
        ...
        describe("phrase", () => {
            itShouldScore("", null, "", []);
            itShouldScore("foo", null, "f fo foo oo o", [
                { ngrams: ["foo"], range: [5, 8] }
            ]);
            itShouldScore("foo", null, "foo fooo ffoo", [
                { ngrams: ["foo"], range: [0, 3] }
            ]);
            itShouldScore("foo", null, "foo foo", [
                { ngrams: ["foo"], range: [0, 3] },
            ...
            itShouldScore('"foo bar"', null, "foo bar foo bar", [
                { ngrams: ["foo", "bar"], range: [0, 7] },
        ...
        describe("clause", () => {
            itShouldScore("fo*", null, "f fo foo foobar", [
                { ngrams: ["fo"], range: [2, 4] },
        ...
        describe("expression not", () => {
            itShouldScore("foo - bar", "any", "foo bar", [
                { ngrams: ["foo"], range: [0, 3] }
            ]);
            itShouldScore("foo - bar", "any", "foo biz", [
                { ngrams: ["foo"], range: [0, 3] },
                { "@not": true, ngrams: ["bar"], range: [0, 3] }
            ]);
            itShouldScore("foo - bar", "any", "bar", []);

            itShouldScore("foo - bar", "all", "foo bar", []);
            itShouldScore("foo - bar", "all", "foo biz", [
                { ngrams: ["foo"], range: [0, 3] },
                { "@not": true, ngrams: ["bar"], range: [0, 3] }
            ]);
            itShouldScore("foo - bar", "all", "bar", []);
        });

        describe("expression and", () => {
            itShouldScore("foo + bar", null, "foo", []);
            itShouldScore("foo + bar", null, "bar", []);
            itShouldScore("foo + bar", null, "foo bar", [
                { ngrams: ["foo"], range: [0, 3] },
        ...
        describe("nesting", () => {
            itShouldScore("foo bar biz", null, "foo biz", [
                { ngrams: ["foo"], range: [0, 3] },
                { ngrams: ["biz"], range: [4, 7] }
            ]);
            itShouldScore("foo bar biz", null, "bar biz", [
                { ngrams: ["bar"], range: [0, 3] },
                { ngrams: ["biz"], range: [4, 7] }
            ]);
            itShouldScore("foo bar biz", null, "foo bar", [
                { ngrams: ["foo"], range: [0, 3] },
                { ngrams: ["bar"], range: [4, 7] }
            ]);
            itShouldScore("foo bar biz", null, "foo bar biz", [
                { ngrams: ["foo"], range: [0, 3] },
                { ngrams: ["bar"], range: [4, 7] },
