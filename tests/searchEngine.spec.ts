    ...
    return new SearchEngine(
        new SearchBackend<People>(peopleSchemaService, () => [
            peopleToStoredDocument({
                ...
                fullName: "foo",
                income: 400,
                addresses: [{ parts: "adr1", kind: "home" }]
            }),
            peopleToStoredDocument({
                id: "2",
                fullName: "bar",
                income: 700,
                addresses: [{ parts: "adr2", kind: "home" }]
            }),
            peopleToStoredDocument({
                id: "3",
                fullName: "biz",
                income: 400,
                addresses: [{ parts: "adr3", kind: "work" }]
            }),
            peopleToStoredDocument({
                id: "4",
                fullName: "buzz",
                income: 70,
                addresses: [{ parts: "adr4", kind: "home" }]
            })
        ]),
        new Scorer(peopleSchemaService, [], null)
    );
...
describe("SearchEngine", () => {
    ...
    describe("highlights", () => {
        it("should produce search highlights", () => {
            ...
            expect(highlights["fullName"]).toEqual([
                "<em>f o</em> o",
                "f <em>o</em> o",
                "f o <em>o</em>"
            ]);
    ...
    describe("limiter", () => {
        ...
        it("should provide info for next page when more than one page", () => {
            ...
            expect(results["@search.nextPageParameters"]).toEqual({
                search: "1|3|5|7|9",
                skip: 1000,
                top: 500
            });
            expect(results["@odata.nextLink"]).toBe(
                "search=1%7C3%7C5%7C7%7C9&%24skip=1000&%24top=500"
            );
        });
    });
    ...
    describe("filter", () => {
        ...
        it("should raise score as matches gets more complex", () => {
            ...
            const results = sut.search({
                filter: "fullName eq 'bar' or id lt 3 and id gt 1"
            });
    ...
    describe("facets", () => {
        ...
        it("should sort facets by query", () => {
            ...
            const results = sut.search({
                facets: ["income,sort:-count", "addresses/kind,sort:-value"]
            });
        ...
        it("should apply facet params in order", () => {
            ...
            const results = sut.search({
                facets: ["income,sort:-count,count:2"]
            });
    ...
    describe("scoring", () => {
        it("should use scoring strategies when available", () => {
            ...
            const inProfile = sut.search({
                search: "foo",
                scoringProfile: "doubleFullName"
            });
            const outProfile = sut.search({
                search: "home",
                scoringProfile: "doubleFullName"
            });
