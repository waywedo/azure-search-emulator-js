...
describe("query-select", () => {
    describe("apply", () => {
            ...
            for (const k in candidate) {
                ...
                if (Array.isArray(candidate[k])) {
                    candidate[k] = candidate[k].map((r: any) =>
                        isPojo(r)
                            ? stripCompiledDeep(r as SimpleParserResult)
                            : r
                    );
        ...
        const quoteAst = (value: string): AstQuote => ({
            type: "QUOTE",
            value
        });
        const partialAst = (value: AstPhrase): AstPartial => ({
            type: "PARTIAL",
            value
        });
        const expressionAst = (
            ...
            op: "not" | "and" | "or",
            right: AstExpression
        ): AstExpression => ({ type: "EXPRESSION", left, op, right });
        ...
        test("foo-bar", expressionAst(wordAst("foo"), "not", wordAst("bar")));
        test("foo+bar", expressionAst(wordAst("foo"), "and", wordAst("bar")));
        test("foo|bar", expressionAst(wordAst("foo"), "or", wordAst("bar")));
        test("foo bar", expressionAst(wordAst("foo"), "or", wordAst("bar")));

        test(
            "foo+bar|biz",
            expressionAst(
                expressionAst(wordAst("foo"), "and", wordAst("bar")),
                "or",
                wordAst("biz")
            )
        );
        ...
        test(
            ...
            expressionAst(
                wordAst("foo"),
                "and",
                expressionAst(wordAst("bar"), "or", wordAst("biz"))
            )
        );
        ...
        test(
            ...
            expressionAst(
                expressionAst(
                    expressionAst(wordAst("foo"), "not", wordAst("bar")),
                    "not",
                    wordAst("biz")
                ),
                "or",
        ...
        test(
            ...
            expressionAst(
                expressionAst(wordAst("foo"), "not", wordAst("bar")),
                "not",
                expressionAst(wordAst("biz"), "or", wordAst("buz"))
            )
        );
    });
});
