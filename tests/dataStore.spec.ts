...
describe("DataStore", () => {
    describe("postDocuments", () => {
        describe("upload", () => {
            it("should add the document with new key", () => {
                const sut = new DataStore<People>(
                    peopleSchemaService,
                    peopleAnalyzer
                );
                    ...
                    value: [
                        {
                            ...
                            id: "abc",
                            fullName:
                                "Mister Mac'Lown de Montreal, protector of (de) tests",
                            income: 500,
                            addresses: [
                                {
                                    kind: "home",
                ...
                expect(sut.documents[0]).toEqual({
                    ...
                    analyzed: {
                        ...
                        "addresses/location": {
                            ...
                            points: [makeGeoPoint(1, 2), makeGeoPoint(3, 4)]
                        }
                    }
                });
            });
    ...
    describe("loadDocuments", () => {
        it("should load documents into an empty data store", () => {
            const sut = new DataStore<People>(
                peopleSchemaService,
                peopleAnalyzer
            );
            ...
            expect(sut.documents).toEqual([
                {
                    key: "a",
                    original: { id: "a", fullName: "test" },
                    analyzed: expect.anything()
                },
                {
                    key: "b",
                    original: { id: "b", fullName: "test2" },
                    analyzed: expect.anything()
                }
            ]);
        });

        it("should fail to load documents into a non-empty data store", () => {
            const sut = new DataStore<People>(
                peopleSchemaService,
                peopleAnalyzer
            );
            ...
            sut.postDocuments({
                value: [{ "@search.action": "upload", id: "a" }]
            });

            const action = () =>
                sut.loadDocuments([{ id: "b", fullName: "test2" }]);

            expect(action).toThrowError(
                "Cannot load documents into a non-empty data store."
            );
        });
    });
});
